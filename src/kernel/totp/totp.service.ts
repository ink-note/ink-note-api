...
export class TotpService {
  ...
  public async createTOTPDetails(serviceName: string, identifier: string): Promise<TOTPDetails> {
    ...
    this.logger.log(`Generated secret key for service: ${serviceName}`);
  ...
  public verifyTOTP(code: string, secret: string): boolean {
    this.logger.log(`Verifying TOTP code for secret: ${secret}`);
    ...
    this.logger.log(`TOTP verification result for secret: ${secret} - ${isValid ? 'Valid' : 'Invalid'}`);
    return isValid;
  }

  public CREATE_ADMIN_OTP_CODE(secret: string) {
    this.logger.log(`Creating admin OTP code for secret: ${secret}`);

    const otpCode = authenticator.generate(secret);
    this.logger.log(`Generated admin OTP code for secret: ${secret}: ${otpCode}`);
    return otpCode;
  }
  ...
  private async createQRCode(value: string, options: QRCodeOptions = {}) {
    this.logger.log(`Creating QR code for value: ${value}`);

    const qrCode = await toDataURL(value, options);
    this.logger.log(`Generated QR code for value: ${value}`);
    return qrCode;
  }
}
